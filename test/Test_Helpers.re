...
type vendor = {
  .
  [@bs.get] "vendor_name": string,
  [@bs.get] "fruits_id": int,
};
              ...
              (
                _ =>
                  ...
                  (
                    _ =>
                      query(
                        "INSERT INTO fruits VALUES (default, 'blueberry')",
                        client,
                      )
                      |> then_ @@
                      (
                        _ =>
                          query(
                            "INSERT INTO fruits VALUES (default, 'raspberry')",
                            client,
                          )
                          |> then_ @@
                          (
                            _ =>
                              query(
                                "INSERT INTO vendors VALUES ('bill',  1)",
                                client,
                              )
                              |> then_ @@
                              (
                                _ =>
                                  query(
                                    "INSERT INTO vendors VALUES ('frank', 2)",
                                    client,
                                  )
                                  |> then_ @@
                                  (
                                    _ =>
                                      query(
                                        "INSERT INTO vendors VALUES ('frank', 3)",
                                        client,
                                      )
                                      |> then_ @@
                                      (
                                        _ =>
                                          query(
                                            "INSERT INTO vendors VALUES ('jane',  4)",
                                            client,
                                          )
                                      )
                                  )
...
let teardown = client =>
  ...
  (
    _ =>
      query("DROP TABLE IF EXISTS fruits", client)
      |> then_ @@
      (_ => end_(client))
  );
